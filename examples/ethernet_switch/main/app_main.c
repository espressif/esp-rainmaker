...
void app_main()
{
    ...
    /* Initialize network interfaces. Note that, these should be called before esp_rmaker_node_init()
     */
#ifdef CONFIG_EXAMPLE_ENABLE_WIFI
    /* Initialize Wi-Fi. This enables Wi-Fi provisioning via phone app */
    app_network_init();
#endif
    /* Initialize Ethernet */
    ESP_ERROR_CHECK(app_ethernet_init());
     ...
     * Note that this should be called after network initialization but before network start()
     * */
    ...
    /* Enable system service (reboot and factory reset only; no WiFi reset) */
    ...
    /* Set PoP for Local Control and on-network challenge-response (from Kconfig) */
    esp_rmaker_local_ctrl_set_pop(CONFIG_EXAMPLE_LOCAL_CTRL_POP);

    /* Initialize on-network challenge-response for user-node mapping */
    app_on_network_chal_resp_init();
     ...
     * after a connection has been successfully established or provisioning is done.
     */
    err = app_network_start(POP_TYPE_RANDOM);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Could not start Wi-Fi. Continuing with Ethernet only...");
        /* Don't abort - Ethernet might still work */
    }
#endif

    /* Start the Ethernet connection.
     * This will wait for Ethernet connection and IP address.
     * RainMaker will use whichever network (Wi-Fi or Ethernet) connects first.
     */
    err = app_ethernet_start();
    if (err != ESP_OK) {
#ifdef CONFIG_EXAMPLE_ENABLE_WIFI
        ESP_LOGW(TAG, "Could not start Ethernet. Wi-Fi may be available as fallback.");
        /* Don't abort if Wi-Fi is enabled - Wi-Fi might still work */
#else
