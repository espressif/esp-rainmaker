...
def main():
    ...
    parser.add_argument('--gw-firmware', type=str, required=False)
    ...
    with open(args.target_file, 'wb') as fout:
        ...
        if args.gw_firmware:
            offset = append_subfile_header(fout, FILETAG_GW_OTA_IMAGE, os.path.getsize(args.br_firmware), offset)
        ...
        if args.gw_firmware:
            append_subfile(fout, args.gw_firmware)
