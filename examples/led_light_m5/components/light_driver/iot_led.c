...
#define LED_ERROR_CHECK(con, err, format, ...) do { \
        if (con) { \
            if(*format != '\0') \
...
esp_err_t iot_led_init(ledc_timer_t timer_num, ledc_mode_t speed_mode, uint32_t freq_hz)
{
    ...
    LED_ERROR_CHECK(ret != ESP_OK, ret, "LEDC timer configuration");
...
{
    esp_err_t ret = ESP_OK;
    LED_ERROR_CHECK(g_light_config == NULL, ESP_FAIL, "iot_led_init() must be called first");
#ifdef CONFIG_SPIRAM_SUPPORT
    LED_ERROR_CHECK(gpio_num != GPIO_NUM_16 || gpio_num != GPIO_NUM_17, ESP_ERR_INVALID_ARG,
    ...
    LED_ERROR_CHECK(ret != ESP_OK, ret, "LEDC channel configuration");
...
{
    LED_ERROR_CHECK(g_light_config == NULL, ESP_FAIL, "iot_led_init() must be called first");
    LED_ERROR_CHECK(dst == NULL, ESP_ERR_INVALID_ARG, "dst should not be NULL");
...
{
    LED_ERROR_CHECK(g_gamma_table == NULL, ESP_FAIL, "iot_led_init() must be called first");
