...
static int light_layer  = 1;
esp_err_t led_light_show_layer()
{
    switch (light_layer) {
        case 1:
...
esp_err_t led_light_set_value(uint16_t cid, int value)
{
    switch (cid) {
        ...
        default:
            ESP_LOGE(TAG, "No support cid: %d", cid);
            return ESP_FAIL;
    }

    ESP_LOGD(TAG, "cid: %d, value: %d", cid, value);

    return ESP_OK;
}

esp_err_t led_light_get_value(uint16_t cid, int *value)
{
    ...
    ESP_LOGI(TAG, "cid: %d, value: %d", cid, *value);
