...
esp_err_t esp_rmaker_cmd_response_publish(void *output, size_t output_len)
{
    if (output) {
        ...
        /* Handler deferred the response (returned ESP_ERR_NOT_FINISHED) or produced an empty one;
         * the handler is expected to call esp_rmaker_cmd_async_response_send() later in that case.
         */
        ESP_LOGD(TAG, "No output to publish now.");
    }
    return ESP_OK;
}
                                             ...
                                             const void *response, size_t response_len)
{
    ...
    void *output = NULL;
    size_t output_len = 0;
    esp_err_t err = esp_rmaker_cmd_prepare_payload(ctx->req_id, 0, status, ctx->cmd,
                                                   response, response_len, &output, &output_len);
    if (err != ESP_OK) {
        return err;
    }
